using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using DevelopTools;

public class CorpsePool : SingleObjectPool<DestCorpse>
{
    [SerializeField] private DestCorpse particle;
    public bool soundPoolPlaying = false;

    public void Configure(DestCorpse _particle)
    {
        extendible = true;
        particle = _particle;
    }

    protected override void AddObject(int prewarm = 3)
    {
        var newParticle = Instantiate(particle);
        newParticle.Initialize();
        newParticle.gameObject.SetActive(false);
        newParticle.transform.SetParent(transform);
        objects.Enqueue(newParticle);
    }

    public void ReturnParticle(DestCorpse particle)
    {
        if (particle == null) return;

        particle.transform.SetParent(transform);
        ReturnToPool(particle);
    }
}
